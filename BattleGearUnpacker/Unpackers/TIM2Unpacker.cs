...
using System.IO;
using System.Xml;
using static BattleGearUnpacker.Formats.TIM2;

namespace BattleGearUnpacker.Unpackers
{
    public static class TIM2Unpacker
    {
        public static void Unpack(string path, TIM2 file)
        {
            string filename = Path.GetFileName(path);
            string extensionless = Path.GetFileNameWithoutExtension(filename);
            string? folder = Path.GetDirectoryName(path);
            if (string.IsNullOrEmpty(folder))
                throw new FriendlyException($"Could not get folder name for: {path}");
            ...
            var xw = XmlWriter.Create(Path.Combine(folder, $"{extensionless}_tim2.xml"), xws);
            xw.WriteStartElement("tim2");
        ...
        public static void Repack(string path)
        {
            ...
            xml.Load(path);
            string decoder = xml.ReadString("tim2/decoder");
            if (decoder != Program.ProgramName)
                Console.WriteLine($"Unrecognized decoder: {decoder}");

            string outName = xml.ReadString("tim2/filename");
            file.FormatVersion = xml.ReadByte("tim2/formatversion");
            file.FormatID = (FormatAlignment)xml.ReadByte("tim2/formatid");

            var picturesNode = xml.SelectNodes("tim2/pictures/picture");
