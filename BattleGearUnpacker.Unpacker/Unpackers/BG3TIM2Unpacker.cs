using BattleGearUnpacker.Core.Formats;
using BattleGearUnpacker.Core.Images;
using BattleGearUnpacker.Unpacker.Exceptions;
using BattleGearUnpacker.Unpacker.Helpers;
using BattleGearUnpacker.Unpacker.Parsing.Xml;
using System;
using System.Collections.Generic;
using System.IO;
using System.Xml;
using static BattleGearUnpacker.Core.Formats.BG3TIM2;

namespace BattleGearUnpacker.Unpacker.Unpackers
{
    public static class BG3TIM2Unpacker
    {
        public static void Unpack(string path, string folder, BG3TIM2 file)
        {
            ...
            for (int picIndex = 0; picIndex < file.Pictures.Count; picIndex++)
            {
                ...
                var png = new PngImage(picture.Width, picture.Height, bitDepth, hasAlpha, indexed, picture.Clut, picture.Image);
                png.Write(picOutPath);
                ...
                if (picture.Mipmaps.Count > 0)
                {
                    ...
                    for (int mipIndex = 0; mipIndex < picture.Mipmaps.Count; mipIndex++)
                    {
                        ...
                        png = new PngImage(picture.Width >>> level, picture.Height >>> level, bitDepth, hasAlpha, indexed, picture.Clut, picture.Mipmaps[mipIndex]);
                        png.Write(picOutPath);
                ...
                if (picture.SubImages.Count > 0)
                {
                    ...
                    foreach (var subimage in picture.SubImages)
                    {
                        ...
                        picOutPath = Path.Combine(folder, outName);
                        png = new PngImage(subimage.Width, subimage.Height, bitDepth, hasAlpha, indexed, picture.Clut, subimage.GetPixels(picture.Image, picture.Width));
                        png.Write(picOutPath);
        ...
        public static void Repack(string folder, string outFolder)
        {
            ...
            string decoder = xml.ReadString("bg3tim2/decoder");
            ...
            if (picturesNode != null)
            {
                foreach (XmlNode pictureNode in picturesNode)
                {
                    ...
                    // Handle sub-images stored in user data
                    ...
                    if (subImagesNode != null)
                    {
                        ...
                        foreach (XmlNode subImageNode in subImagesNode)
                        {
                            ...
                            {
                                png.ReadSubImage(subImagePath, subImageX, subImageY);
                            }
                    ...
                    png.HasAlpha = picture.HasAlpha;
                    png.IsIndexed = picture.Indexed;
                    png.BitDepth = bitDepth;
                    png.Normalize();
                    picture.Image = png.Image;
                    picture.Clut = png.Palette;
                    ...
                    if (mipmapTextures > 1)
                    {
                        ...
                        var mipMapFilenameNodes = pictureNode.SelectNodes("mipmaps/filenames/filename");
                        if (mipMapFilenameNodes != null)
                        {
                            foreach (XmlNode mipMapFilenameNode in mipMapFilenameNodes)
                            {
                                // Read PNG
                                // TODO: Check clut and transform data if necessary
                                pngName = mipMapFilenameNode.InnerText;
                                pngPath = Path.Combine(folder, pngName);
                                if (!File.Exists(pngPath))
                                    throw new FriendlyException($"Could not find mipmap PNG: {pngPath}");
                                var mipmapPng = PngImage.Read(pngPath);
                                mipmapPng.HasAlpha = picture.HasAlpha;
                                mipmapPng.IsIndexed = picture.Indexed;
                                mipmapPng.BitDepth = picture.BitDepth;
                                mipmapPng.Normalize();
                                mipmapPng.NormalizeToPalette(png.Palette);
                                picture.Mipmaps.Add(mipmapPng.Image);
                            }
                        }
                    }
